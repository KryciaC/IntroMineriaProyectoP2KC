...
dataNATRATA2 <- subset (dataNAsexo, !is.na(TRATARECIB))

dataNATRATA3 <- subset (datasexo, !is.na(TRATARECIB))
```
...
arbolt3 <- rpart(TRATARECIB ~
                  ...
                  CONDIEGRES+
                  SEXO,
                data = dataNATRATA2, method = "class")
```

```{r}
#Se grafica el árbol utilizando rpart.plot:
rpart.plot(arbolt3, type=2, extra=0, under = TRUE, fallen.leaves = TRUE, box.palette = "BuGn", 
           main ="Predicción de tratamiento recibido", cex = 0.5)
```
```{r}
#Se procede a crear el árbol para predecir la variable ÁREA para lo cual se utiliza rpart, se indica la variable a predecir, las variables a utilizar para la predicción, el dataset y el método a utilizar: 
arbolt4 <- rpart(TRATARECIB ~
                  AÑO+
                  DIASESTANCIA+
                  PPERTENENCIA+
                  EDADAÑOS+
                  ...
                  CONDIEGRES+
                  SEXO,
                data = dataNATRATA3, method = "class")
```

```{r}
#Se grafica el árbol utilizando rpart.plot:
rpart.plot(arbolt4, type=2, extra=0, under = TRUE, fallen.leaves = TRUE, box.palette = "BuGn", 
           main ="Predicción de tratamiento recibido", cex = 0.5)
```
...
```{r}
#Para estos análisis se considerarán unicamente los años entre 2009 y 2017 debido a los cambios de categorías a partir de 2018, de igual forma para tener solamente 2 valores vamos a eliminar los valores 9= ignorado:
